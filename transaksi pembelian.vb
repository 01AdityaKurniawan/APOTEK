...
Public Class transaksi_pembelian
    ...
    Sub StokObat()
        Koneksi.Open()
        Dim query As String = "SELECT TP.ID_Obat, O.Nama_Obat, O.Harga_Obat, O.jenis_obat, TP.Jumlah
                          FROM Tabel_Pembelian TP 
                          INNER JOIN Tabel_Obat O ON TP.ID_Obat = O.ID_Obat"
        Dim command = New SqlCommand(query, Koneksi.connection)
        Dim reader As SqlDataReader = command.ExecuteReader()

        ' Memasukkan data ke dalam tabel obat
        While reader.Read()
                Dim obatID As Integer = CInt(reader("ID_Obat"))
                Dim namaObat As String = reader("Nama_Obat").ToString()
                Dim hargaObat As Decimal = CDec(reader("Harga_Obat"))
                Dim jenisObat As String = reader("Jenis_Obat").ToString()
                Dim jumlah As Integer = CInt(reader("Jumlah"))

            ' Menjalankan query untuk memasukkan data ke dalam tabel obat
            Dim insertQuery As String = "INSERT INTO Tabel_Obat (ID_Obat, Nama_Obat, Harga_Obat, Jenis_Obat, Jumlah) " &
                                    "VALUES (@ID_Obat, @Nama_Obat, @Harga_Obat, @Jenis_Obat, @Jumlah)"

            Koneksi.Open()
            Dim insertCommand As New SqlCommand(insertQuery, Koneksi.connection)
            insertCommand.Parameters.AddWithValue("@ID_Obat", obatID)
                insertCommand.Parameters.AddWithValue("@Nama_Obat", namaObat)
            insertCommand.Parameters.AddWithValue("@Harga_Obat", hargaObat)
            insertCommand.Parameters.AddWithValue("@Jenis_Obat", jenisObat)
